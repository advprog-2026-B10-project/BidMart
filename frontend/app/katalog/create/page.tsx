...
export default function CreateCatalogPage() {
  ...
  // Ambil email dari localstorage buat dijadiin sellerId
  const sellerEmail = useSyncExternalStore(
      () => () => {},
  ...
  const [hargaBeliSekarang, setHargaBeliSekarang] = useState(''); // Kita anggap ini Reserve Price ya
  const [durasiLelang, setDurasiLelang] = useState('1440'); // Default 1 hari (1440 menit)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Siapin data buat Katalog
      const payloadKatalog = {
        judul,
        deskripsi,
      ...
      // 2. Tembak API Katalog punya temen lu
      const responseKatalog = await axiosClient.post('/katalog', payloadKatalog);
      const newListingId = responseKatalog.data.id; // Dapet ID-nya nih!

      // 3. Langsung combo tembak API Bidding punya lu buat bikin lelang (DRAFT)
      const payloadBidding = {
        listingId: newListingId,
        startingPrice: Number(hargaAwal),
        reservePrice: hargaBeliSekarang ? Number(hargaBeliSekarang) : Number(hargaAwal) * 1.5, // Kalau kosong, set default reserve price
        durationInMinutes: Number(durasiLelang),
        sellerId: sellerEmail,
        type: 'ENGLISH' // Default tipe lelang
      };

      // Pastiin URL ini sesuai sama config axiosClient lu (biasanya ngarah ke /api)
      await axiosClient.post('/bidding/create', payloadBidding);

      alert('Listing dan Mesin Lelang berhasil dibuat (Status: DRAFT)!');
      // Diarahin ke halaman edit biar dia bisa publish nanti
      router.push(`/katalog/${newListingId}/edit`);

    } catch (err: any) {
      console.error(err);
      setError(err.response?.data?.message || 'Gagal membuat listing barang atau mesin lelang.');
    } finally {
          ...
          <div className="bg-gray-800 p-8 rounded-2xl border border-gray-700 shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Harga Cadangan/Reserve (Rp) <span className="text-red-500">*</span></label>
                  <input
                      type="number"
                      required
                      min="0"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Durasi Lelang (Menit) <span className="text-red-500">*</span></label>
                  <input
                      type="number"
                      required
                      min="1"
                      value={durasiLelang}
                      onChange={(e) => setDurasiLelang(e.target.value)}
                      className="w-full bg-gray-900 border border-gray-600 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-blue-500 transition"
                      placeholder="1440"
                  />
                </div>
              ...
              >
                {isSubmitting ? 'Menyimpan...' : 'Simpan Draft Lelang'}
              </button>
            </form>
          </div>
        </div>
