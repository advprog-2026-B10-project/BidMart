...
export default function CatalogDetailPage() {
  ...
  const handleRedirectToAuction = async () => {
    try {
      const res = await axiosClient.get('/bidding/auctions', {
        headers: { Authorization: `Bearer ${token}` }
      });
      const auctions = res.data;

      const targetAuction = auctions.find((a: any) => a.listingId === Number(id));

      if (targetAuction) {
        router.push(`/auction/${targetAuction.id}`);
      } else {
        alert('Belum ada mesin lelang untuk barang ini!');
      }
    } catch (err) {
      console.error('Gagal mencari lelang', err);
  ...
  const handlePublish = async () => {
    if (!confirm('Yakin ingin mem-publish lelang ini? Setelah publish, barang langsung bisa di-bid.')) return;

    try {
      // Cari ID lelangnya dulu dari listingId
      const res = await axiosClient.get('/bidding/auctions', {
        headers: { Authorization: `Bearer ${token}` }
      });
      const targetAuction = res.data.find((a: any) => a.listingId === Number(id));

      if (!targetAuction) {
        alert('Mesin lelang belum dibuat untuk barang ini!');
        return;
      }

      // Tembak API backend buat ngubah status jadi ACTIVE
      await axiosClient.put(`/bidding/auctions/${targetAuction.id}/activate`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });

      // Cukup kasih alert sukses aja, user tetep stay di halaman ini
      alert('Lelang berhasil dipublish dan sekarang berstatus ACTIVE! 🚀');

    } catch (err: any) {
      console.error('Gagal mem-publish lelang', err);
      alert(err.response?.data?.message || 'Gagal mem-publish lelang. Pastikan backend sudah di-update!');
    }
  };
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      if (targetAuction) {
        await axiosClient.delete(`/bidding/auctions/${targetAuction.id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
      }
      ...
      alert('Listing dan lelang berhasil dihapus sepenuhnya dari sistem! 🗑️');
      router.push('/katalog');

    } catch (err: any) {
      console.error('Gagal membatalkan listing', err);
      alert(err.response?.data?.message || err.response?.data || 'Gagal membatalkan listing.');
    }
  };
          ...
          <div className="mb-6 flex justify-between items-center">
            ...
            {userRole === 'SELLER' && (
                <div className="flex gap-3">
                  {/* 🔥 TOMBOL PUBLISH DITAMBAH DI SINI 🔥 */}
                  <button
                      onClick={handlePublish}
                      className="px-4 py-2 bg-green-600 hover:bg-green-700 rounded-lg text-sm font-semibold shadow-lg shadow-green-900/20 transition"
                  >
                    Publish Lelang
                  </button>
            ...
            <div className="p-8 w-full md:w-1/2 flex flex-col">
              ...
              {userRole === 'BUYER' && (
                  <button
                      ...
                      className="mt-6 w-full py-3 bg-blue-600 hover:bg-blue-700 font-bold rounded-xl transition shadow-lg shadow-blue-900/20 text-lg"
                  >
                    Ajukan Tawaran (Bid)
                  </button>
              )}
